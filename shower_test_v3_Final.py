...
from relay_b import Relay  # Import the Relay class
from matplotlib.backends.backend_qtagg import FigureCanvasQTAgg as FigureCanvas
from matplotlib.figure import Figure

## READ ME ##
# Change part of "yolov8n.pt" to custom model
# Check IP Of Relay standard is 192.168.1.200 or 192.168.1.254
# Jetson orin must be config same IP gateway
## IO Connect ##
# Trigger Start = DI pin 1
# Trigger Finish = DI pin 2
# Result OK = Channel 4 ON (1sec)
# Result Off = Channel 5 Off (1sec)
# Modify OK and NG class at line
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("AGC Water Leakage Detection System")
    ...
    def finish_inspection(self):
        ...
        if sum(self.camera_ng_flags.values()) > 0:
            ...
            self.relay.on(5) #Turn Relay5 On when NG Found
        else:
            self.total_ok_count += 1  # Increment OK counter only if no NG was detected
            self.relay.on(4) #Turn Relay4 On When OK
        ...
        while True:
            ...
            if ret:
                ...
                if self.inspection_running:
                    ...
                    if "person" in detected_classes and not self.camera_ng_flags[position]:  # "class2" is NG Modify This "person" to "Leak"
                    ...
                    elif "class1" in detected_classes:  # "class1" is OK Modify this to "class that = OK"
                        self.update_status(position, "OK", "green")
                    else:
                        self.update_status(position, "Checking. . .", "gray")
            else:
    ...
    def update_graph(self):
        ...
        # Add grid to the grap
        ax.grid(True, which="both", linestyle="--", linewidth=0.5)

        ax.set_title("Inspection Cycle Time")
