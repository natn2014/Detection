...
class VideoOCRApp(QWidget):
    ...
    def browse_json_file(self):
        ...
        sanitized_barcode = scan_barcode.split("$")[0]
    ...
    def populate_table(self, data, table, detected_texts=None):
        ...
        if detected_texts is not None:
            if found_not_found:
                ...
                Relay.off(self.relay, 5)
            if found_match_or_partial:
                print("on relay 4")
                Relay.on(self.relay, 4)
            else:
                Relay.off(self.relay, 4)
    ...
    def save_setting_data(self):
        ...
        if ok and model_name:
            # Sanitize model name: remove '$' and everything after
            sanitized_model_name = model_name.split("$")[0]
